...
this.dispatch = function(req, res) {
  ...
  if (req.url === "/" || req.url === "") {
    fs.readFile(__dirname + '/../webroot/layout/homepage.html', function(error, content) {
      if (error) {
    ...
    if (typeof actions[action] == 'function') {
		if (argument) {
      	...
      	fs.readFile(__dirname + '/../webroot/' + req.url, function(error, content) {
      		if (error) {
      			serverError(404, 'Resource Not Found');
      		}
			else {
				renderHtml('text/javascript', content);
			}
      	})
    }
  }
